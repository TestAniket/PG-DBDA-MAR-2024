...
df = df.filter(df.unit_selling_price != 0)

df = df.dropna(subset=["total_weighted_landing_price"])

# Calculate the total sales per product
df = df.withColumn(
    "Total_Sales",
    F.col("procured_quantity"))
...
df = df.withColumn(
    "Discount_Percentage", 
     F.round((F.col("total_discount_amount") / (F.col("unit_selling_price") * F.col("procured_quantity"))) * 100))

# Calculate the profit margin for each row
df = df.withColumn(
    "Profit_Margin", 
     F.round(((((F.col("procured_quantity") * F.col("unit_selling_price")) - F.col("total_discount_amount")) - F.col("total_weighted_landing_price")) / 

...
class Employee:
    def __init__(self,root):
        ...
        self.employeeTable = ttk.Treeview(DBFrame,columns=('PID','dep','name','desig',
                                                           'email','add','MS','DOB','DOJ',
                                                           'Gen','Phone','Country','Sal',)
    ...
    def addData(self):
        if self.varPanId.get()=="" or self.varName.get()=="" or self.varDept.get()=="":
            ...
            try:
                ...
                addCursor.execute("insert into employee values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                                                                                                           self.varPanId.get(),
                                                                                                           self.varDept.get(),
                                                                                                           self.varName.get(),
    ...
    def fetchData(self):
        ...
        self.searchOption.set("Search By")
    ...
    def getCursor(self,event=""):
        ...
        data = content['values']
        self.varPanId.set(data[0])
        self.varDept.set(data[1])
        self.varName.set(data[2])
        self.varDesignation.set(data[3])
        self.varEmail.set(data[4])
        self.varAdd.set(data[5])
        self.varMS.set(data[6])
        self.varDOB.set(data[7])
        self.varDOJ.set(data[8])
    ...
    def searchData(self):
        if self.searchInput.get()==' ' or self.searchOption.get()=='Search By':
            ...
            try:
                ...
                if self.searchOption.get() == "PanNo":
                    query = "SELECT * FROM employee WHERE PanNo = %s"
                else:
                    query = "SELECT * FROM employee WHERE Contact = %s"

                searchCursor.execute(query, (self.searchInput.get(),))

                # # query='SELECT * FROM employee WHERE %s = %s'.format(self.searchOption.get(),self.searchInput.get())
                # # searchCursor.execute(query, (self.searchOption.get(),self.searchInput.get()))
